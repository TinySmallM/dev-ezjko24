...
class StorageController extends Controller
{
    ...
    public function actionUpload(){
        ...
        $m = Member::find()->where(['token_hash'=>Yii::$app->request->post('token_hash')])->one();
        if(!$m) return ['error'=>'Пользователь не найден'];

        setlocale (LC_ALL, array ('ru_RU.utf-8', 'rus_RUS.utf-8')); //Basename cyrillic fix

        $typeValid = ['image/png','image/jpg','image/jpeg','image/bmp'];
        ...
        foreach ($_FILES['file']['error'] as $key=>$val) {
            ...
            if ( $_FILES['file']['error'][$key] == 0 && in_array($_FILES['file']['type'][$key],$typeValid) ) {
                ...
                $newName = $file['filename'].'_'.substr( md5(time().basename($f['name'])),0,10 );

                //return $file['extension'];

                /*if($file['extension'] == 'svg'){
                    move_uploaded_file($_FILES['file']['tmp_name'][$key],$uploads_dir.'thumb_'.$newName.'.'.$file['extension']);
                    return ['files'=>[
                        ['file'=>$newName.'.'.$file['extension'],'thumb'=>'thumb_'.$newName.'.'.$file['extension']]
                    ]];
                }
                */
