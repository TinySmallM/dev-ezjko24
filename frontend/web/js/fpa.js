...
let fingerprint = {
    ...
    canvas: null,
    cookie: fingerprint_cookie()?1:null,
    display: fingerprint_display(),
    language: fingerprint_language(),
    os: fingerprint_os(),
    timezone: fingerprint_timezone(),
    touch: fingerprint_touch()?1:null,
    useragent: fingerprint_useragent(),
    ya_client_id: null,
    url: window.location.href 
}
/*
ym(70515631, 'getClientID', function(clientID) {
    fingerprint.ya_client_id = clientID;
});
*/
...
setTimeout(function(){
	...
	axios.post('/fingerprint/api?type=collect', fingerprint)
	.then(function (res) {
		res = res.data;
	    console.log(res);
	})
